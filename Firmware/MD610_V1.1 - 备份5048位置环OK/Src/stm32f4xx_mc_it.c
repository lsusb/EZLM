...
//    TSK_HighFrequencyTask();          /*GUI, this section is present only if DAC is disabled*/          //lhq×¢ÊÍ
		if(Init_OK)
		{
				Foc_Control(); 
//				TSK_HighFrequencyTask();
		}
		if(	mc_svpm.AdcData.Calibration == 1)					
		{
				mc_svpm.AdcData.offeset[PhaseA] += ( uint16_t )( ( ADC1->JDR1 ) >> 3 );
				mc_svpm.AdcData.offeset[PhaseB] += ( uint16_t )( ( ADC2->JDR1 ) >> 3 );
				mc_svpm.AdcData.sample_cnt ++;
				if(mc_svpm.AdcData.sample_cnt == 50)
					mc_svpm.AdcData.Calibration = 0;				//½áÊøADCÐ£×¼²É¼¯
		}
...
//    ICS_BRK_IRQHandler(&PWM_Handle_M1);				//lhq×¢ÊÍ
  }
...
//    MC_RunMotorControlTasks();                  //lhq×¢ÊÍ
  /* USER CODE BEGIN SysTick_IRQn 2 */
  /* USER CODE END SysTick_IRQn 2 */
}

/**
  * @brief  This function handles Button IRQ on PIN PA3.
  */
void EXTI3_IRQHandler (void)
{
...
//    UI_HandleStartStopButton_cb ();             //lhq×¢ÊÍ
  }
